...
function Navigator(props) {
  ...
  const _applyToApp = async (settings, allSetting) => {
    ...
    /** APP INTRO */
    if (settings.intro_screen && settings.intro_screen.is_show_intro_screen) {
      let appIntro = await Helpers.getDataStorage(Keys.AS_APP_INTRO);
      if (appIntro && appIntro !== "") {
        // Do nothing
        let checkRoleUser = await Helpers.getDataStorage(Keys.AS_DATA_USER);
        if (checkRoleUser && checkRoleUser !== "") {
          checkRoleUser = JSON.parse(checkRoleUser)
          if (checkRoleUser.role === Configs.USER_ROLE.STORE_MANAGER) {
            initRoute = "VendorTab";
            dispatch(UserActions.updateUser(checkRoleUser));
          }
        }
      } else initRoute = "AppIntro";
    } else {
      let checkRoleUser = await Helpers.getDataStorage(Keys.AS_DATA_USER);
      if (checkRoleUser && checkRoleUser !== "") {
        checkRoleUser = JSON.parse(checkRoleUser)
